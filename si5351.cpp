...
void Si5351::init(uint8_t xtal_load_c, uint32_t ref_osc_freq)
{
	...
	Wire.setDefaultTimeout(SI5351_TIMEOUT); // added 3/9/2017 by TLB, prevents code hanging when not connected
...
void Si5351::set_ms(enum si5351_clock clk, struct Si5351RegSet ms_reg, uint8_t int_mode, uint8_t r_div, uint8_t div_by_4)
{
	...
	switch(clk)
	{
		...
		case SI5351_CLK7:
			...
			break;
		case SI5351_CLKNONE: // added 3/13/2017 by TLB: added to fix compiler warning
			break;
	}
...
{
	//uint8_t reg_val, reg;
	uint8_t reg_val; // added 3/13/2017 by TLB: fixes compiler warning due to reg not being used
	reg_val = si5351_read(SI5351_CLK0_CTRL + (uint8_t)clk);
...
uint8_t Si5351::si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	// added 3/13/2017 by TLB: now returns status from endTransmission
...
{
	//uint8_t reg_val;
	uint8_t reg_val = 0; // changed 3/13/2017 by TLB: initialized to 0 to fix compiler warning
...
void Si5351::ms_div(enum si5351_clock clk, uint8_t r_div, uint8_t div_by_4)
{
	...
	uint8_t reg_val = 0, reg_addr = 0; // changed 3/13/2017 by TLB: initialized to 0 to fix compiler warning
